...
st.info("网页默认提供一组示例数据，用户可直接修改；所有标星项目均为必填。公开版计算器不启用缺失值自动插补，输入不完整时不会输出预测结果。")
...
if submitted:
    try:
        ...
        height_value = required_float(height, "身高（m）")
        weight_value = required_float(weight, "体重（kg）")
        fat_mass_value = required_float(fat_mass, "体脂量（kg）")
        muscle_mass_value = required_float(muscle_mass, "肌肉量（kg）")
        ati_value = required_float(ati, "躯干倾斜角 ATI（°）")
        ka_value = required_float(ka, "脊柱后凸角 KA（°）")
        act_values = {
            var: required_float(value, ACT_LABEL_MAP_CN.get(var, var))
            for var, value in act_inputs.items()
        }
        bal_values = {
            var: required_float(value, BAL_LABEL_MAP_CN.get(var, var))
        ...
        st.write("- 所有标星数值项是否完整填写且合理；本计算器不进行缺失值插补")
        st.exception(e)
