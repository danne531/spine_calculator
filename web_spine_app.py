...
st.info(
    "The webpage provides one default example case that users can edit directly. All starred fields are required. The public calculator does not perform automatic missing-value imputation; predictions are not generated when inputs are incomplete."
)
...
if submitted:
    try:
        ...
        height_value = required_float(height, "Height (m)")
        weight_value = required_float(weight, "Weight (kg)")
        fat_mass_value = required_float(fat_mass, "Fat mass (kg)")
        muscle_mass_value = required_float(muscle_mass, "Muscle mass (kg)")
        ati_value = required_float(ati, "Angle of trunk inclination (°)")
        ka_value = required_float(ka, "Kyphos angle (°)")
        act_values = {
            var: required_float(value, ACT_LABEL_MAP_EN.get(var, var))
            for var, value in act_inputs.items()
        }
        bal_values = {
            var: required_float(value, BAL_LABEL_MAP_EN.get(var, var))
        ...
        st.write("- Whether all starred numeric inputs are complete and reasonable; this calculator does not impute missing values")
        st.exception(e)
