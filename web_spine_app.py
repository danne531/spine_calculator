...
with st.form("patient_form"):
    ...
    with col3:
        height = st.text_input("Height (m)", value=f"{DEFAULT_HEIGHT:.1f}", placeholder=" ")
    with col4:
        weight = st.text_input("Weight (kg)", value=f"{DEFAULT_WEIGHT:.1f}", placeholder=" ")
    with col5:
        fat_mass = st.text_input("Fat mass (kg)", value=f"{DEFAULT_FAT_MASS:.1f}", placeholder=" ")
    with col6:
        muscle_mass = st.text_input("Muscle mass (kg)", value=f"{DEFAULT_MUSCLE_MASS:.1f}", placeholder=" ")

    col7, col8, col9, col10 = st.columns(4)
    with col7:
        ati = st.text_input("Angle of trunk inclination (°)", value=str(example_int("ATI", 5)), placeholder=" ")
    with col8:
        ka = st.text_input("Kyphos angle (°)", value=str(example_int("KA", 33)), placeholder=" ")
    with col9:
    ...
    for i, var in enumerate(activity_vars()):
        ...
        with cols_act[i % 4]:
            act_inputs[var] = st.text_input(
                label, value=str(example_int(var, 0)), placeholder=" ", key=f"act_{var}"
            )
    ...
    for i, var in enumerate(balance_vars()):
        ...
        with cols_bal[i % 3]:
            bal_inputs[var] = st.text_input(
                label, value=str(example_int(var, 0)), placeholder=" ", key=f"bal_{var}"
            )
