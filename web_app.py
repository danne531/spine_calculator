...
with st.form("patient_form"):
    ...
    with col3:
        height = st.text_input("身高（m）", value=f"{DEFAULT_HEIGHT:.1f}", placeholder=" ")
    with col4:
        weight = st.text_input("体重（kg）", value=f"{DEFAULT_WEIGHT:.1f}", placeholder=" ")
    with col5:
        fat_mass = st.text_input("体脂量（kg）", value=f"{DEFAULT_FAT_MASS:.1f}", placeholder=" ")
    with col6:
        muscle_mass = st.text_input("肌肉量（kg）", value=f"{DEFAULT_MUSCLE_MASS:.1f}", placeholder=" ")

    col7, col8, col9, col10 = st.columns(4)
    with col7:
        ati = st.text_input("躯干倾斜角 ATI（°）", value=str(example_int("ATI", 5)), placeholder=" ")
    with col8:
        ka = st.text_input("脊柱后凸角 KA（°）", value=str(example_int("KA", 33)), placeholder=" ")
    with col9:
