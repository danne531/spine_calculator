...
st.info("未填写的数值项将在计算前采用随机森林插补方法补全；已填写的项目将保持不变。")
...
with st.form("patient_form"):
    ...
    with col1:
        gender_cn = st.selectbox(required_label("性别"), ["男", "女"], index=0)
    with col2:
        age = st.selectbox(required_label("年龄（岁）"), list(range(13, 19)), index=0)
    ...
    with col3:
        height = st.number_input(
            ...
            value=None, step=0.1, format="%.1f", placeholder=" "
        )
    with col4:
    ...
    with col7:
        ati = st.number_input(
            ...
            value=None, step=1, format="%d", placeholder=" "
        )
    with col8:
        ka = st.number_input(
