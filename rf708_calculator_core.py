...
def _predict_rf_model(rf_model: dict[str, Any], x: dict[str, Any]) -> Any:
    if rf_model["type"] == "classification":
        votes = {cls: 0 for cls in rf_model["classes"]}
        for tree in rf_model["trees"]:
            votes[_predict_tree_value(tree, x, rf_model)] += 1
        return max(rf_model["classes"], key=lambda cls: votes.get(cls, 0))

    preds = [_predict_tree_value(tree, x, rf_model) for tree in rf_model["trees"]]
    return float(np.mean(preds))


def apply_random_forest_imputer(raw_row: dict[str, Any]) -> dict[str, Any]:
    """Replicate the R calculator's missForest-style imputation for one web input row."""
    imp = ASSETS["imputer"]
    pred_names = imp["pred_names"]
    numeric_vars = set(imp["numeric_vars"])
    factor_vars = set(imp["factor_vars"])
    medians = imp["medians"]
    modes = imp["modes"]
    factor_levels = imp["factor_levels"]

    x: dict[str, Any] = {}
    raw_missing: dict[str, bool] = {}
    for var in pred_names:
        value = raw_row.get(var)
        raw_missing[var] = _is_missing(value)
        if var in numeric_vars:
            x[var] = _to_float(value, float(medians[var]))
        elif var in factor_vars:
            levels = [str(v) for v in factor_levels[var]]
            mode = str(modes[var])
            val = mode if raw_missing[var] else str(value)
            x[var] = val if val in levels else mode

    for target, rf_model in imp["rf_models"].items():
        if not raw_missing.get(target, False):
            continue
        predictors = {name: x[name] for name in rf_model["predictor_names"]}
        predicted = _predict_rf_model(rf_model, predictors)
        if target in numeric_vars:
            x[target] = float(predicted)
        elif target in factor_vars:
            levels = [str(v) for v in factor_levels[target]]
            mode = str(modes[target])
            pred_str = str(predicted)
            x[target] = pred_str if pred_str in levels else mode
...
def add_pc1(raw_row: dict[str, Any]) -> tuple[dict[str, Any], dict[str, float], dict[str, Any]]:
    imputed = apply_random_forest_imputer(raw_row)
    ...
    return row_with_pc, pc_values, imputed


def _derive_bmi(height: Any, weight: Any) -> float | None:
...
def _derive_fmr(fat_mass: Any, muscle_mass: Any) -> float | None:
...
def predict_patient(raw_row: dict[str, Any]) -> dict[str, Any]:
    row_with_pc, pc_values, imputed = add_pc1(raw_row)
