...
st.info(
    "The webpage provides one default example case that users can edit directly. Numeric fields cleared and left blank will be imputed using a random forest imputation procedure before prediction. Values entered by the user will remain unchanged."
)
...
with st.form("patient_form"):
    ...
    with col3:
        height = st.number_input("Height (m)", 1.0, 2.0, value=DEFAULT_HEIGHT, step=0.1, format="%.1f", placeholder=" ")
    with col4:
        weight = st.number_input("Weight (kg)", 20.0, 120.0, value=DEFAULT_WEIGHT, step=0.1, format="%.1f", placeholder=" ")
    with col5:
        fat_mass = st.number_input("Fat mass (kg)", 1.0, 80.0, value=DEFAULT_FAT_MASS, step=0.1, format="%.1f", placeholder=" ")
    with col6:
        muscle_mass = st.number_input("Muscle mass (kg)", 1.0, 80.0, value=DEFAULT_MUSCLE_MASS, step=0.1, format="%.1f", placeholder=" ")

    col7, col8, col9, col10 = st.columns(4)
    with col7:
        ati = st.number_input("Angle of trunk inclination (°)", 0, 40, value=example_int("ATI", 5), step=1, format="%d", placeholder=" ")
    with col8:
        ka = st.number_input("Kyphos angle (°)", 0, 90, value=example_int("KA", 33), step=1, format="%d", placeholder=" ")
    with col9:
