...
def example_label(text: str, example) -> str:
    return f"{text} :gray[(e.g. {example})]"
...
with st.form("patient_form"):
    ...
    with col1:
        gender_text = st.selectbox(required_label("Sex"), ["Select", "Male", "Female"], index=0)
    with col2:
        age = st.selectbox(required_label("Age (years) :gray[(e.g. 13)]"), ["Select"] + list(range(6, 26)), index=0)

    st.subheader("2. Body composition and sagittal/coronal alignment")
    col3, col4, col5, col6 = st.columns(4)
    with col3:
        height = st.number_input(example_label("Height (m)", "1.6"), 1.0, 2.0, value=None, step=0.1, format="%.1f", placeholder="e.g. 1.6")
    with col4:
        weight = st.number_input(example_label("Weight (kg)", "50.0"), 20.0, 120.0, value=None, step=0.1, format="%.1f", placeholder="e.g. 50.0")
    with col5:
        fat_mass = st.number_input(example_label("Fat mass (kg)", "10.0"), 1.0, 80.0, value=None, step=0.1, format="%.1f", placeholder="e.g. 10.0")
    with col6:
        muscle_mass = st.number_input(example_label("Muscle mass (kg)", "30.0"), 1.0, 80.0, value=None, step=0.1, format="%.1f", placeholder="e.g. 30.0")

    col7, col8, col9, col10 = st.columns(4)
    with col7:
        ati = st.number_input(example_label("Angle of trunk inclination (°)", "6"), 0, 40, value=None, step=1, format="%d", placeholder="e.g. 6")
    with col8:
        ka = st.number_input(example_label("Kyphos angle (°)", "35"), 0, 90, value=None, step=1, format="%d", placeholder="e.g. 35")
    with col9:
        st_label = st.selectbox(required_label("Scoliosis type"), ["Select"] + list(ST_OPTIONS_EN.keys()), index=0)
    with col10:
        sct_label = st.selectbox(
            required_label("Sagittal curvature abnormality type (SCT)"),
            ["Select"] + list(SCT_OPTIONS_EN.keys()),
            index=0,
        )
    ...
    for i, var in enumerate(activity_vars()):
        ...
        with cols_act[i % 4]:
            act_inputs[var] = st.number_input(
                example_label(label, example_int(var)), value=None, step=1, format="%d",
                placeholder=f"e.g. {example_int(var)}", key=f"act_{var}"
            )
    ...
    for i, var in enumerate(balance_vars()):
        ...
        with cols_bal[i % 3]:
            bal_inputs[var] = st.number_input(
                example_label(label, example_int(var)), value=None, step=1, format="%d",
                placeholder=f"e.g. {example_int(var)}", key=f"bal_{var}"
            )
...
if submitted:
    try:
        if gender_text == "Select":
            st.error("Please select sex.")
            st.stop()
        if age == "Select":
            st.error("Please select age.")
            st.stop()
        if st_label == "Select" or sct_label == "Select":
            st.error("Please select both scoliosis type and sagittal curvature abnormality type (SCT).")
            st.stop()
        ...
        height = 1.6 if height is None else float(height)
        weight = 50.0 if weight is None else float(weight)
        fat_mass = 10.0 if fat_mass is None else float(fat_mass)
        muscle_mass = 30.0 if muscle_mass is None else float(muscle_mass)
        ati = 6 if ati is None else int(ati)
        ka = 35 if ka is None else int(ka)
